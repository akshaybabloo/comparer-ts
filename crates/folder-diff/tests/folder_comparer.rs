use folder_diff::{
    ChangeReason, ChangeStatus, DiffError, EntryKind, FolderComparer, FolderDiff, FsEntry, Hasher,
    Side, TreeNode,
};
...
fn compare_with(
    ...
    let mut comparer = FolderComparer::new(left, right).unwrap();
...
fn different_sizes_are_modified_without_hashing() {
    let comparer = FolderComparer::new(vec![file("a.txt", 1)], vec![file("a.txt", 2)]).unwrap();
...
fn equal_sizes_are_hashed_on_both_sides() {
    let comparer = FolderComparer::new(vec![file("a.txt", 4)], vec![file("a.txt", 4)]).unwrap();
    let jobs: Vec<(u32, Side, &str, u64)> = comparer
        .jobs()
        .iter()
        .map(|job| (job.id, job.side, job.path.as_str(), job.size))
        .collect();

    assert_eq!(
        jobs,
        vec![(0, Side::Left, "a.txt", 4), (1, Side::Right, "a.txt", 4)]
    );
}
...
fn different_special_files_are_a_kind_change() {
    ...
    let comparer = FolderComparer::new(vec![fifo], vec![socket]).unwrap();
...
fn a_hash_that_was_never_set_is_unknown() {
    let comparer = FolderComparer::new(vec![file("a", 4)], vec![file("a", 4)]).unwrap();
...
fn rejects_invalid_listings() {
    ...
    for (listing, message) in cases {
        let error = FolderComparer::new(vec![], listing)
            .err()
...
#[test]
fn rejects_unknown_job_ids() {
    let mut comparer = FolderComparer::new(vec![], vec![]).unwrap();

    assert_eq!(comparer.set_hash(0, "abc"), Err(DiffError::UnknownJob(0)));
    assert_eq!(
        comparer.set_error(7, "nope").unwrap_err().to_string(),
        "unknown hash job 7"
    );
}
